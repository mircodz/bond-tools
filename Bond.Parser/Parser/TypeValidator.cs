...
/// <summary>
/// Validates resolved types and field defaults, expanding aliases with their type arguments.
/// </summary>
public static class TypeValidator
{
    public static void ValidateType(BondType type, SourceLocation location)
    {
        switch (type)
        {
            ...
            case BondType.List list:
                ValidateType(list.ElementType, location);
                break;
            case BondType.Vector vector:
                ValidateType(vector.ElementType, location);
                break;
            case BondType.Nullable nullable:
                ValidateType(nullable.ElementType, location);
                break;
            case BondType.Maybe maybe:
                ValidateType(maybe.ElementType, location);
                break;
    ...
    public static bool ValidateDefaultValue(BondType fieldType, Default? defaultValue)
    {
        ...
        return (fieldType, defaultValue) switch
        {
            ...
            (BondType.Float, Default.Float or Default.Integer) => true,
            (BondType.Double, Default.Float or Default.Integer) => true,
            (BondType.Bool, Default.Bool) => true,
            (BondType.String, Default.String) => true,
            (BondType.WString, Default.String) => true,
            (BondType.TypeReference { Declaration: EnumDeclaration enumeration }, Default.Enum enumDefault) =>
                enumeration.Constants.Any(constant => constant.Name == enumDefault.Identifier),
